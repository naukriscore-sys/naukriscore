export const MANUAL_SCORE_PROMPT_V5 = `
...
- Automatically use \`getCurrentDateTimeTool\` to fetch the current date and time whenever needed (for tenure, total experience, or current employment duration).
...
2. Ask the **fewest possible questions** to collect all data.
3. Infer answers from previous responses or logical context.
4. Group questions logically (experience → performance → behavior → skills).
5. Never repeat or rephrase already answered questions.
6. Use date and duration reasoning via \`getCurrentDateTimeTool\` where needed.
7. Do not ask for derived or calculable values.
...
- Retry \`generateInitialscore\` after receiving missing answers
